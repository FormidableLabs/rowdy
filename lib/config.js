...
module.exports = function (cfg) {
  ...
  // --------------------------------------------------------------------------
  // SauceLabs: Perform browser object substitutions (Guacamole)
  // --------------------------------------------------------------------------

  // If guacamole is enabled, insert sauceLabs browser objects where needed.
  if (cfg.settings && cfg.settings.guacamoleEnabled) {
    if (settingsPath.length === 2 && settingsPath[0] === "sauceLabs") {
      //
      // TODO: Fix this path
      //
      guacamole.useShrinkwrap("../../../guacamole-shrinkwrap.json");
      ...
      if (!cfg.settings.sauceLabs) {
        cfg.settings.sauceLabs = {};
      }
      // Insert coniguration
      cfg.settings.sauceLabs[browserId] = {desiredCapabilities: matchingCapabilities[0] };
    }
  }
